...
// VISTA PREVIA — copia de GoogleMapsLotMap.tsx con geolocalización en tiempo real
// (punto azul del visitante + botón "Centrar en mí"). NO se usa en la home;
// se renderiza sólo en /mapa-preview. El componente de producción queda intacto.
...
type GeoStatus = 'idle' | 'locating' | 'tracking' | 'denied' | 'unsupported' | 'error'

export default function GoogleMapsLotMapGeo({ lots }: { lots: Lot[] }) {
  ...
  const lastPos = useRef<google.maps.LatLngLiteral | null>(null)
  ...
  // Geolocalización en tiempo real del visitante (punto azul + círculo de precisión)
  useEffect(() => {
    if (!loaded) return
    ...
    const id = navigator.geolocation.watchPosition(
      pos => {
        ...
        if (!userMarker.current) {
          ...
          userMarker.current.setPosition(p)
          accuracyCircle.current?.setCenter(p)
          accuracyCircle.current?.setRadius(pos.coords.accuracy)
        }
  ...
  const centerOnMe = useCallback(() => {
    if (lastPos.current && mapObj.current) {
      mapObj.current.panTo(lastPos.current)
      mapObj.current.setZoom(19)
    }
  }, [])

  const GEO_MSG: Record<GeoStatus, string> = {
    idle: '',
    locating: 'Buscando tu ubicación…',
    tracking: 'Ubicación en tiempo real activa',
    denied: 'Permiso de ubicación denegado',
    unsupported: 'Este dispositivo no soporta geolocalización',
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="mb-10 text-center">
          <span className="mb-3 inline-block rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-widest text-white" style={{ backgroundColor: '#FF1200' }}>
            Vista previa · Ubicación en tiempo real
          </span>
          <h2 className="text-4xl font-black text-gray-900">Recorré el loteo con geolocalización</h2>
          <p className="mt-3 text-gray-500 text-sm">
            Abrí esta página desde el celular, en el predio, y el punto azul te muestra en qué lote estás parado
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-5" style={{ minHeight: 560 }}>
          ...
          <div className="lg:col-span-3 flex flex-col gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={centerOnMe}
                disabled={geoStatus !== 'tracking'}
                className="ml-auto flex items-center gap-1.5 rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors disabled:opacity-40"
            ...
            {geoStatus !== 'idle' && (
              <p className="text-xs" style={{ color: geoStatus === 'tracking' ? '#16a34a' : geoStatus === 'locating' ? '#6B6660' : '#dc2626' }}>
                {GEO_MSG[geoStatus]}
              </p>
            )}
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="rounded-2xl border p-5" style={{ background: '#fff', borderColor: '#D8D2C7' }}>
              <p className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: '#8A6A47' }}>Cómo usar el punto azul</p>
              <div className="space-y-3 text-sm" style={{ color: '#2E2A26' }}>
                <p>1. Abrí esta página desde el celular, parado en el predio.</p>
                <p>2. Permití el acceso a tu ubicación cuando el navegador lo pida.</p>
                <p>3. El punto azul se mueve con vos y muestra sobre qué lote estás.</p>
                <p className="text-xs" style={{ color: '#6B6660' }}>Requiere conexión HTTPS y GPS activado. La precisión mejora al aire libre.</p>
              </div>
            </div>
